	...
	**/
	...
	function updateGrid():Void
	{
		...
		for (i in sectionInfo)
		{
			...
			if (daSus > 0)
			{
				var sustainVis:FlxSprite = new FlxSprite(note.x + (note.width / 4),
					note.y + (note.height / 2)).makeGraphic(8,
						Math.floor(Math.abs(Math.floor(FlxMath.remapToRange(daSus, 0, Conductor.stepCrochet * 16, 0, gridBG.height))) * 1.0));
				sustainVis.frames = note.frames;
				sustainVis.animation.copyFrom(note.animation);
				sustainVis.animation.play('hold');
				sustainVis.alpha = 0.6;
				sustainVis.shader = note.shader;

				sustainVis.setGraphicSize(note.width / 2, Math.floor(Math.abs(Math.floor(FlxMath.remapToRange(daSus, 0, Conductor.stepCrochet * 16, 0, gridBG.height))) * 1.0));
