...
import ume.backend.SongConductor.BPMChangeEvent;
...
class ChartingState extends MusicBeatState {
	...
	var curRenderedNotes:FlxTypedGroup<GameNote>;
	...
	**/
	...
	override function create() {
		...
		curRenderedNotes = new FlxTypedGroup<GameNote>();
		...
		loadSong(_song.song);
		SongConductor.changeBPM(_song.bpm);
	...
	function addSongUI():Void {
		...
		stepperBPM.value = SongConductor.bpm;
	...
	function addSectionUI():Void {
		...
		stepperSectionBPM = new FlxUINumericStepper(10, 80, 1, SongConductor.bpm, 1, 999, 3);
		stepperSectionBPM.value = SongConductor.bpm;
	...
	function addNoteUI():Void {
		...
		stepperSusLength = new FlxUINumericStepper(10, 10, SongConductor.stepCrochet / 2, 0, 0, SongConductor.stepCrochet * 16);
	...
	function loadSong(daSong:String):Void {
		...
		FlxG.sound.playMusic(UMEAssets.inst(daSong), 0.6);

		// WONT WORK FOR TUTORIAL OR TEST SONG!!! REDO LATER
		vocals = new FlxSound().loadEmbedded(UMEAssets.voices(daSong.toLowerCase()));
	...
	override function update(elapsed:Float) {
		...
		strumLine.y = getYfromStrum((SongConductor.time - sectionStartTime()) % (SongConductor.stepCrochet * _song.notes[curSection].lengthInSteps));
		...
		if (!typingShit.hasFocus) {
			...
			if (FlxG.mouse.wheel != 0) {
				...
				FlxG.sound.music.time -= (FlxG.mouse.wheel * SongConductor.stepCrochet * 0.4);
		...
		/* if (FlxG.keys.justPressed.UP)
				SongConductor.changeBPM(SongConductor.bpm + 1);
		...
		bpmTxt.text = bpmTxt.text = Std.string(FlxMath.roundDecimal(SongConductor.time / 1000, 2))
			+ " / "
	...
	function recalculateSteps():Int {
		...
		curStep = lastChange.stepTime + Math.floor((FlxG.sound.music.time - lastChange.songTime) / SongConductor.stepCrochet);
		updateBeat();
	...
	function updateGrid():Void {
		...
		for (i in sectionInfo) {
			...
			var note:GameNote = new GameNote(daStrumTime, daNoteInfo % 4);
			note.sustainLength = daSus;
			note.setGraphicSize(GRID_SIZE, GRID_SIZE);
			note.updateHitbox();
			note.x = Math.floor(daNoteInfo * GRID_SIZE);
			note.y = Math.floor(getYfromStrum((daStrumTime - sectionStartTime()) % (SongConductor.stepCrochet * _song.notes[curSection].lengthInSteps)));
