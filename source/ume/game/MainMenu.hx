...
class MainMenu extends MusicBeatState {
	...
	override function create() {
		...
		var bg:FlxSprite = new FlxSprite(-80).loadGraphic(UMEAssets.image('menu/menuBG'));
		...
		magenta = new FlxSprite(-80).loadGraphic(UMEAssets.image('menu/menuBGMagenta'));
	...
	override function update(elapsed:Float):Void {
		...
		if (!selectedSomethin) {
			if (controls.UI_UP_P) {
				FlxG.sound.play(UMEAssets.sound('scrollMenu'));
				changeItem(-1);
			}
			...
			if (controls.BACK) {
				...
				FlxG.sound.play(UMEAssets.sound('cancelMenu'));
				FlxG.switchState(new Title());
			}
			if (controls.ACCEPT) {
				selectedSomethin = true;
				FlxFlicker.flicker(magenta, 1.1, 0.15, false);
				FlxG.sound.play(UMEAssets.sound('confirmMenu'));
