...
import ume.objects.FlxSparrow;

class Freeplay extends MusicBeatSubState {
	var funnyCamera:FlxCamera;
	var leftBar:FlxSprite = new FlxSprite(-FlxG.width / 2, 0).makeGraphic(Std.int(FlxG.width / 2), FlxG.height, FlxColor.GRAY).loadGraphic(UMEAssets.image('freeplay/pinkBack'));
	var rightBar:FlxSprite = new FlxSprite(FlxG.width, 0).makeGraphic(Std.int(FlxG.width / 2), FlxG.height, FlxColor.BLACK).loadGraphic(UMEAssets.image('freeplay/freeplayBGdad'));
